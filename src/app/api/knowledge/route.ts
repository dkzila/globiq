...
const ENDPOINTS = [
  ...
  { method: 'GET', path: '/api/knowledge/page/{slug}?country=&language=', auth: null, purpose: 'The assembled §22 knowledge page: quick fact, deeper representations by format, sources, related concepts, exam-coverage mirror (P3-S5), canonical path + translations (P2-S5)' },
...
export async function GET() {
  return ok({
    ...
    readingPage: '§22 layers: quickFact → representations (format-aware) → sources → related → examCoverage (P3-S5: which exams need this unit today)',
    endpoints: ENDPOINTS,
  })
}
