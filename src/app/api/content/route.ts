/**
 * GET /api/content — endpoint index for the ContentItem layer of the knowledge
 * module (Master Plan §37). ContentItems are publishable representations of a
 * KnowledgeUnit (§7): one language × one format each, with immutable published
 * revisions (§36).
 */
...
const ENDPOINTS = [
  ...
  { method: 'GET', path: '/api/content/items/{id}', auth: null, purpose: 'Public content detail — always the live revision snapshot (§36)' },
...
export async function GET() {
  return ok({
    ...
    spec: 'Master Plan §7 (representation of a canonical record), §19 (immutable at revision level), §23 (formats), §35 (per-country languages), §36 (revisions)',
    ...
    publicVisibility: 'PUBLISHED items of VERIFIED units (live revision only)',
    endpoints: ENDPOINTS,
  })
}
